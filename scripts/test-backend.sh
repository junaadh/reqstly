...
TEST_DATABASE_PASSWORD_ENCODED="${TEST_DATABASE_PASSWORD_ENCODED:-${POSTGRES_PASSWORD_ENCODED:-}}"

if [[ -z "${TEST_DATABASE_PASSWORD_ENCODED}" ]]; then
  TEST_DATABASE_PASSWORD_ENCODED="$(
    python3 - "${TEST_DATABASE_PASSWORD}" <<'PY'
import sys
import urllib.parse
...
export TEST_DATABASE_ADMIN_URL="${TEST_DATABASE_ADMIN_URL:-postgres://postgres:${TEST_DATABASE_PASSWORD_ENCODED}@${TEST_DATABASE_HOST}:${TEST_DATABASE_PORT}/${TEST_DATABASE_NAME}}"
